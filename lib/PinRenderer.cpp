...
void PinRenderer::render(const RenderState *state) {
    ...
    for (size_t i = 0; i < m_pins.size(); ++i) {
        ...
        float scale =
            30.0F * m_headScale * (distToCam / Utils::CAMERA_DISTANCE); // increased from 15 to 30
        ...
        rub->updateDynamicBuffer(m_topUniformBuffer, i * UNIFORM_ALIGNMENT, sizeof(TopUniformData),
                                 &topUbuf);
    }
    ...
    if (m_topVertexBuffer) {
        const QRhiCommandBuffer::VertexInput vb[] = {{m_topVertexBuffer, 0}};
        cb->setVertexInput(0, 1, vb);
...
void PinRenderer::updateBuffers(QRhi *rhi, QRhiResourceUpdateBatch *rub) {
    ...
    rub->updateDynamicBuffer(m_vertexBuffer, 0, vertexData.size() * sizeof(Vertex),
                             vertexData.data());
}
...
void PinRenderer::initializeRHI(QRhi *rhi) {
    ...
    m_topShaderBindings->setBindings({QRhiShaderResourceBinding::uniformBufferWithDynamicOffset(
        0, QRhiShaderResourceBinding::VertexStage | QRhiShaderResourceBinding::FragmentStage,
        m_topUniformBuffer, sizeof(TopUniformData))});
...
void PinRenderer::createPipeline(QRhi *rhi) {
    ...
    m_topPipeline->setShaderStages({{QRhiShaderStage::Vertex, topVertexShader},
