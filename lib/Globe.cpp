...
void Globe::addPin(qreal lat, qreal lon, const QString &label) {
  constexpr float TARGET_ALTITUDE = 1.1F; // Target altitude (10% above surface)
  PinData pin;
  pin.lat = static_cast<float>(lat);
  pin.lon = static_cast<float>(lon);
  pin.altitude = TARGET_ALTITUDE;
  pin.text = label;
  ...
  m_pinsChanged = true;
  m_newSmokeSources.push_back(
      {static_cast<float>(lat), static_cast<float>(lon), TARGET_ALTITUDE});
  update();
}

void Globe::addMarker(qreal lat, qreal lon, const QString &label,
                      bool connected) {

  constexpr float TARGET_ALTITUDE = 1.2F;
  MarkerData mk;
  mk.lat = static_cast<float>(lat);
  mk.lon = static_cast<float>(lon);
  mk.altitude = TARGET_ALTITUDE;
  mk.text = label;
...
void Globe::scheduleUpdate() { update(); }
...
void Globe::updateState() {
  ...
  const float dt =
      static_cast<float>(currentTimeMs - m_lastFrameTime) / 1000.0F;
  m_lastFrameTime = currentTimeMs;

  // Calculate MVP here so we can project labels to 2D
  const float cameraAngle =
      static_cast<float>(m_rotationOffset) + std::numbers::pi_v<float> +
      ((2.0F * std::numbers::pi_v<float> * currentTime) / m_dayLength);
  const float dist = Utils::CAMERA_DISTANCE / static_cast<float>(m_scale);

  QVector3D cameraPos(dist * std::cos(cameraAngle), m_viewAngle * dist,
  ...
  for (auto &sat : m_satellites) {
    if (sat.progress < 1.0F) {
      ...
      sat.progress = std::min(sat.progress, 1.0F);
      m_satellitesChanged = true;
    }
  }
  ...
  for (auto &pinItem : m_pins) {
    if (pinItem.progress < 1.0F) {
      ...
      pinItem.progress = std::min(pinItem.progress, 1.0F);
      m_pinsChanged = true;
    }
    ...
    if (!pinItem.text.isEmpty() && pinItem.progress > 0.3F) {
      ...
      if (clipPos.w() > 0.0F) {
        ...
        if (QVector3D::dotProduct(surfacePos.normalized(),
                                  cameraPos.normalized()) > -0.2F) {
          QVariantMap map;
  ...
  // Animate markers and add labels (line duration ~2000ms, marker duration
  // ~500ms)
  for (auto &mk : m_markers) {
    ...
    if (mk.previousIndex >= 0 &&
        mk.previousIndex < static_cast<int>(m_markers.size())) {
      canAnimate = m_markers[mk.previousIndex].lineProgress >= 1.0F;
    }

    if (canAnimate && mk.lineProgress < 1.0F) {
      mk.lineProgress += dt / 2.0F;
      mk.lineProgress = std::min(mk.lineProgress, 1.0F);
      changed = true;
    }
    ...
    if (mk.lineProgress > markerDelay && mk.markerProgress < 1.0F) {
      ...
      mk.markerProgress = std::min(mk.markerProgress, 1.0F);
      changed = true;
    }
...
QSGNode *Globe::updatePaintNode(QSGNode *oldNode, UpdatePaintNodeData *data) {
  ...
  auto *globeNode = static_cast<GlobeRenderer *>(root->firstChild());
  if (!globeNode) {
  ...
  auto *satNode = static_cast<SatelliteRenderer *>(
  ...
  auto *pinNode = static_cast<PinRenderer *>(
  ...
  auto *markerNode = static_cast<MarkerRenderer *>(
  ...
  auto *smokeNode = static_cast<SmokeRenderer *>(
  ...
  auto *introLinesNode = static_cast<IntroLinesRenderer *>(
  ...
  const QRect pixelRect = QRect(
      qRound(screenRect.x() * dpr), qRound(screenRect.y() * dpr),
